...
package org.wso2.carbon.identity.post.authn.handler.resetPasswordEnforcer;
...
public class PasswordChangeHandler extends AbstractEventHandler implements IdentityConnectorConfig {
    ...
    public void handleEvent(Event event) throws IdentityEventException {
        ...
        if (PasswordPolicyConstants.PASSWORD_GRANT_POST_AUTHENTICATION_EVENT.equals(eventName)) {
            handlePasswordExpiryInPasswordGrantType(event, username, userStoreManager);
            return;
        }
        ...
        updateLastPasswordChangedClaim(username, userStoreManager, timestamp);
    }

    private static void updateLastPasswordChangedClaim(String username, UserStoreManager userStoreManager, long timestamp)
    ...
    private void handlePasswordExpiryInPasswordGrantType(Event event, String username, UserStoreManager userStoreManager)
            throws IdentityEventException {
    ...
    public Properties getDefaultPropertyValues(String tenantDomain) throws IdentityGovernanceException {
        ...
        if (StringUtils.isEmpty(passwordExpiryInDays)) {
        ...
        String enableDataPublishing = setEmailNotificationDefaultValue(tenantDomain);

        properties.setProperty(PasswordPolicyConstants.CONNECTOR_CONFIG_ENABLE_EMAIL_NOTIFICATIONS,
                enableDataPublishing);

        properties.setProperty(PasswordPolicyConstants.CONNECTOR_CONFIG_ENABLE_PASSWORD_EXPIRY,
                PasswordPolicyConstants.FALSE);

        setPriorNotice(tenantDomain, properties);

        return properties;
    }

    private static String setEmailNotificationDefaultValue(String tenantDomain) {
        ...
        return enableDataPublishing;
    }

    private static void setPriorNotice(String tenantDomain, Properties properties) {
    ...
    public void init(InitConfig configuration) throws IdentityRuntimeException {
        ...
        EnforcePasswordResetPostAuthenticationHandlerDataHolder.getInstance().getBundleContext().registerService
    ...
    private boolean isPasswordExpired(String tenantDomain, String tenantAwareUsername,
        ...
        if (passwordChangedTime > 0) { // obtain the day difference from last password changed time to current time
            ...
            daysDifference = (int) ((currentTimeMillis - passwordChangedTime) / (1000 * 60 * 60 * 24)); // convert to days
        }
        if (log.isDebugEnabled()) {
