...
import org.wso2.carbon.identity.post.authn.handler.resetPasswordEnforcer.PasswordPolicyUtils;
import org.wso2.carbon.identity.post.authn.handler.resetPasswordEnforcer.internal.EnforcePasswordResetPostAuthenticationHandlerDataHolder;
...
public class PasswordResetEnforcerHandlerTest {
    ...
    public void testHandle() throws Exception {
        ...
        idPs.put(PasswordPolicyTestConstants.LOCAL, authenticatedIdPData);

        List<AuthenticatorConfig> authenticators = getAuthenticatorConfigs();
        PowerMockito.when(passwordChangeHandler, PasswordPolicyTestConstants.IS_PASSWORD_EXPIRY_ENABLED,
                        PasswordPolicyTestConstants.CARBON_SUPER)
                .thenReturn(PasswordPolicyTestConstants.TRUE);
        String[] claimURIs
                = new String[]{PasswordPolicyTestConstants.HTTP_WSO_2_ORG_CLAIMS_IDENTITY_LAST_PASSWORD_UPDATE_TIME};
        Map<String, String> claimValueMap = new HashMap<>();
        claimValueMap.put
                (PasswordPolicyTestConstants.HTTP_WSO_2_ORG_CLAIMS_IDENTITY_LAST_PASSWORD_UPDATE_TIME1, "1677845659");

        RealmService mockRealmService = mock(RealmService.class);
        UserRealm userRealm = mock(UserRealm.class);
        UserStoreManager mockUserStoreManager = mock(UserStoreManager.class);

        EnforcePasswordResetAuthenticationHandler passwordChangeHandler
                = getEnforcePasswordResetAuthenticationHandler(mockRealmService, (UserRealm) userRealm);

        when(userRealm.getUserStoreManager()).thenReturn(mockUserStoreManager);
        when(mockUserStoreManager.getUserClaimValues(PasswordPolicyTestConstants.BOB, claimURIs , null))
                .thenReturn(claimValueMap);

        doReturn(authenticators).when(idPs.get(PasswordPolicyTestConstants.LOCAL)).getAuthenticators();

        when(mockRealmService.getTenantUserRealm(anyInt())).thenReturn(userRealm);
        when(userRealm.getUserStoreManager()).thenReturn(mockUserStoreManager);

        passwordChangeHandler.handle(mock(HttpServletRequest.class),
                mock(HttpServletResponse.class),
                authenticationContext);
    }

    private static EnforcePasswordResetAuthenticationHandler getEnforcePasswordResetAuthenticationHandler
            (RealmService mockRealmService, UserRealm userRealm) throws UserStoreException {
        EnforcePasswordResetAuthenticationHandler passwordChangeHandler
                = PowerMockito.spy(new EnforcePasswordResetAuthenticationHandler());
        EnforcePasswordResetPostAuthenticationHandlerDataHolder dataHolder
                = PowerMockito.mock(EnforcePasswordResetPostAuthenticationHandlerDataHolder.class);
        ...
        when(mockRealmService.getTenantUserRealm(MultitenantConstants.SUPER_TENANT_ID)).thenReturn(userRealm);
